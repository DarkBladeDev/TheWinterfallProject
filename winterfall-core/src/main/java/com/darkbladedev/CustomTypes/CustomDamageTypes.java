...
public class CustomDamageTypes {
    ...
    public static DamageSource createDehydrationDamageSource(Player player) {
        try {
            ...
            if (damageType == null) {
                Bukkit.getConsoleSender().sendMessage(MiniMessage.miniMessage().deserialize(plugin.PREFIX + " <dark_red>Error: <red>No se pudo encontrar el tipo de daño 'dehydration'. Usando daño genérico."));
                return DamageSource.builder(DamageType.GENERIC).build();
            }
